    ...
    <>
      ...
      <AnimatePresence>
        {activeSection && (
          <motion.div
            key={activeSection}
            className="fixed left-1/2 -translate-x-1/2 md:hidden z-40 pointer-events-none"
      ...
      {/* Mobile back-to-top pill — bottom-left to avoid overlap with EN/PL switch */}
      <AnimatePresence>
        {scrolled && (
          <motion.a
            href="#top"
            ...
            style={{ bottom: "calc(1.75rem + env(safe-area-inset-bottom, 0px))", left: "1.25rem" }}
            initial={{ opacity: 0, y: 8, scale: 0.95 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 8, scale: 0.95 }}
